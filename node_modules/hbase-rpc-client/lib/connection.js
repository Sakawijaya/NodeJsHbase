...
(function() {
  var ByteBuffer, Call, ClientService, Connection, ConnectionHeader, DataInputStream, DataOutputBuffer, DataOutputStream, EventEmitter, GetRequest, ProtoBuf, RequestHeader, ResponseHeader, Service, UserInformation, builder, connectionId, debug, hconstants, net, proto, ref, rpcBuilder, rpcProto,
  ...
  ref = require('./output-buffer'), DataOutputStream = ref.DataOutputStream, DataOutputBuffer = ref.DataOutputBuffer;
  ...
  ConnectionHeader = rpcProto.ConnectionHeader, RequestHeader = rpcProto.RequestHeader, ResponseHeader = rpcProto.ResponseHeader, UserInformation = rpcProto.UserInformation;
  ...
  module.exports = Connection = (function(superClass) {
    ...
    function Connection(options) {
      ...
      this._handleClose = bind(this._handleClose, this);
      this.setupConnection = bind(this.setupConnection, this);
      this.writeHead = bind(this.writeHead, this);
      this.processMessages = bind(this.processMessages, this);
      this.setupIOStreams = bind(this.setupIOStreams, this);
      ...
      this._tcpKeepAlive = options.tcpKeepAlive;
      this._realUser = options.realUser;
      this._effectiveUser = options.effectiveUser;
      this.address = {
        host: options.host,
        port: options.port
      };
    ...
    Connection.prototype.setupIOStreams = function() {
      ...
      return this.socket.on('connect', (function(_this) {
        return function() {
          var ch, header, impl, userInfo;
          debug("connected to " + _this.name);
          _this.writeHead();
          userInfo = {};
          if (_this._realUser) {
            userInfo.realUser = _this._realUser;
          }
          userInfo.effectiveUser = _this._effectiveUser;
    ...
    Connection.prototype.processMessages = function(messages) {
      ...
      delete this.calls[header.callId];
      if (header.exception) {
    ...
    Connection.prototype.setupConnection = function() {
      ...
      this.socket.setNoDelay(this._tcpNoDelay);
      this.socket.setKeepAlive(this._tcpKeepAlive);
  ...
  ClientService = (function(superClass) {
    extend(ClientService, superClass);
